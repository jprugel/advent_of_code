...
pub fn create_map(input: &str) -> HashMap<&str, i32> {
    let tokens: Vec<&str> = input.split_whitespace().map(|token| token.trim_matches(',')).collect();
    let keys: Vec<&str> = tokens.iter().filter(|&&token| token.parse::<i32>().is_err()).copied().collect();
    let values: Vec<i32> = tokens.iter().filter_map(|&token| token.parse::<i32>().ok()).collect();
    keys.into_iter().zip(values.into_iter()).collect()
}

pub fn merge_greatest_map(vec: Vec<HashMap<&str, i32>>) -> HashMap<&str, i32> {
    let result: HashMap<&str, i32> = vec
        .into_iter()
...
pub fn get_valid_id(input: &str, validator: [(&str, i32); 3]) -> i32 {
    ...
    let check: bool = merged.iter().all(|(&key, &value)| {
        value <= law[key]
    });
    match check {
        true => game_number,
